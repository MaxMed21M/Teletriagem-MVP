...
def _client(timeout: float = 15.0) -> httpx.Client:
    return httpx.Client(timeout=timeout, base_url=get_api_base(), headers={"Accept": "application/json"})


def create_triage(data: Dict[str, Any]) -> Dict[str, Any]:
    with _client() as client:
        response = client.post("/api/triage/", json=data)
...
def request_ai_triage(data: Dict[str, Any]) -> Dict[str, Any]:
    with _client(timeout=60.0) as client:
        response = client.post("/api/triage/ai", json=data)
        response.raise_for_status()
        return response.json()
...
def list_triages(*, limit: int = 50, source: Optional[str] = None) -> list[Dict[str, Any]]:
    ...
    with _client() as client:
        response = client.get("/api/triage/", params=params)
        response.raise_for_status()
        return response.json()
